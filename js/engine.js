...
var Engine = (function(global) {
    ...
    var doc = global.document,
        ...
        lastTime;
        let bgSong = doc.querySelector('embed');
        bgSong.remove();
        let songBg = doc.getElementById('song');
        
        let chocoboSong = `<embed src='sounds/chocobo.mp3' width="180" height="90" loop="true" autostart="true" hidden="true" />`;
        songBg.innerHTML = chocoboSong;
        //loop a song because somehow the embed cant loop
        setInterval(function(){ 
            songBg.innerHTML = null;
    ...
    function updateEntities(dt) {
        ...
        allRandomSpawned.forEach(function(enemy) {
            enemy.update(dt);
         });
        player.update();
        theWomb.update(dt);
            ...
            case "gameOver":
                ...
                for (row = 0; row < numRows; row++) {
                    for (col = 0; col < numCols; col++) {
                        ...
                        ctx.fillText(`GG`, canvas.width/2, canvas.height/3);
                        ctx.fillStyle = "black";
                        ctx.font = "20px Comic Sans MS";
                        ctx.textAlign = "center";
                        ctx.fillText(`Your Score is`, canvas.width/2.1, canvas.height/2.6);
                        ctx.fillStyle = "red";
                        ctx.font = "30px Comic Sans MS";
                        ctx.fillText(`${player.score}`, canvas.width/1.5, canvas.height/2.6);
                        ctx.fillStyle = "black";
                        ctx.font = "20px Comic Sans MS";
                        ctx.textAlign = "center";
                        ctx.fillText("Press Enter To Restart", canvas.width/2, canvas.height/2.3);
                    }
                }
                break;
                }
